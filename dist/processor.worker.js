! function() {
	...
	r.prototype.initialize = function() {
		...
		var r, i, s = this.outputBufferSize,
			a = this.ratioWeight,
			f = this.lastWeight,
			n = 0,
			o = 0,
		...
		if (e % this.channels == 0) {
			if (e > 0) {
				for (; f < 1; f += a)
					for (n = 1 - (o = f % 1), r = 0; r < this.channels; ++r) l[h++] = this.lastOutput[r] * n + t[
						r] * o;
				for (f--, e -= this.channels, i = Math.floor(f) * this.channels; h < s && i < e;) {
					for (n = 1 - (o = f % 1), r = 0; r < this.channels; ++r) l[h++] = t[i + r] * n + t[i + this
						.channels + r] * o;
		...
		if (r % this.channels == 0) {
			if (r > 0) {
				...
				do {
					if (h)
						for (a = s, f = 0; f < this.channels; ++f) e[f] = 0;
					else {
		...
		return "short16" === self.arrayBufferType && (t = function(t) {
			for (var e = new ArrayBuffer(2 * t.length), r = new DataView(e), i = 0, s = 0; s < t
				.length; s += 1, i += 2) {
		...
		if ("init" === s) {
			var f = a.frameSize,
				...
				o = a.fromSampleRate,
				h = a.arrayBufferType;
			return self.frameSize = f * Math.floor(o / n), self.resampler = new r(o, n, 1), self.frameBuffer = [],
		...
		if ("stop" === s && (self.postMessage({
			...
			var l, u = a;
			if (self.frameSize) return (l = self.frameBuffer).push.apply(l, t(u)), self.frameBuffer.length >= self
				.frameSize && (self.postMessage({
